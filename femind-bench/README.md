# femind-bench (Deprecated)

**This tool has been superseded by [RecallBench](https://github.com/victorysightsound/recallbench).**

All prompt engineering, judge logic, self-verification, and the femind adapter have been ported to RecallBench. Use RecallBench for all future benchmark work:

```bash
cargo install recallbench
recallbench run --system femind --dataset longmemeval --variant oracle
```

## Why RecallBench

- Same prompts and scoring as femind-bench v3 (95.6% result parity)
...
This directory contains the benchmark results from the v1/v2/v3 iteration cycle that brought femind from 87.0% to 95.6% on LongMemEval Oracle. See `../BENCHMARK_PROGRESS.md` for the full history.
