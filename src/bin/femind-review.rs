...
fn parse_scope(flag: &str, value: &str) -> femind::error::Result<ReviewScope> {
    ReviewScope::from_str(value).ok_or_else(|| {
        FemindError::Migration(format!(
            "invalid {flag} value '{value}'. Expected general|production|staging|lab|migration|maintenance."
        ))
    })
}
...
fn parse_policy_class(flag: &str, value: &str) -> femind::error::Result<ReviewPolicyClass> {
    ReviewPolicyClass::from_str(value).ok_or_else(|| {
        FemindError::Migration(format!(
            "invalid {flag} value '{value}'. Expected operational-exception|network-exposure-exception|destructive-maintenance|secret-handling-exception|migration-exception|breakglass-exception|private-infrastructure-exception|auth-bypass-exception|data-reset-exception|traffic-cutover-exception."
        ))
    })
}
...
fn parse_template(flag: &str, value: &str) -> femind::error::Result<ReviewApprovalTemplate> {
    ReviewApprovalTemplate::from_str(value).ok_or_else(|| {
        FemindError::Migration(format!(
            "invalid {flag} value '{value}'. Expected staging-bridge|migration-bridge|lab-exception|breakglass-ops|private-endpoint-bridge|lab-auth-bypass|maintenance-reset|traffic-cutover."
        ))
    })
}
...
Usage:
  ...
  femind-review resolve --database <path> --memory-id <id> --status <pending|allowed|denied|expired> [--note <text>] [--reviewer <name>] [--scope <general|production|staging|lab|migration|maintenance>] [--class <policy-class>] [--template <staging-bridge|migration-bridge|lab-exception|breakglass-ops|private-endpoint-bridge|lab-auth-bypass|maintenance-reset|traffic-cutover>] [--replacement-id <id>] [--expires-at <rfc3339>] [--format <text|json>]
